...
instance (Monad m) => PairingM (CoDO m) DO m where
  pairM f (CoDO list _ _ _ _ _ _)    (ListDroplets k)       = pairM f list k
  pairM f (CoDO _ create _ _ _ _ _)  (CreateDroplet conf k) = pairM f (create conf) k
  pairM f (CoDO _ _ destroy _ _ _ _) (DestroyDroplet i k)   = pairM f (destroy i) k
  pairM f (CoDO _ _ _ action _ _ _)  (DropletAction i a k)  = pairM f (action i a) k
  pairM f (CoDO _ _ _ _  ks _ _)     (ListKeys k)           = pairM f ks k
  pairM f (CoDO _ _ _ _  _ szs _)    (ListSizes k)          = pairM f szs k
  pairM f (CoDO _ _ _ _ _ _ imgs)    (ListImages k)         = pairM f imgs k
